...
/// Code-first view for the JFR JavaFX Events detail page.
public final class JavaFxEventsPaneView {

    private final Label titleLabel = new Label();
    private final Label summaryLabel = new Label();
    private final Label phaseLabel = new Label();
    private final Label pulseLabel = new Label();
    private final Label inputLabel = new Label();
    private final TableView<JavaFxPulsePhase> phaseTable = denseTable();
    private final TableView<JavaFxPulseSummary> pulseTable = denseTable();
    private final TableView<JavaFxInputEvent> inputTable = denseTable();
    private final Label detailTitleLabel = new Label();
    private final TextArea detailArea = textArea();

    public JavaFxEventsPaneView(VBox pane) {
        configureDetailPage(pane, titleLabel, summaryLabel,
                vbox(8, phaseLabel, phaseTable),
                detailTitleLabel, detailArea,
                vbox(8, pulseLabel, pulseTable, inputLabel, inputTable));
    }

    public JavaFxEventsPageView view() {
        return new JavaFxEventsPageView(titleLabel, summaryLabel,
                phaseLabel, phaseTable, detailTitleLabel, detailArea, pulseLabel, pulseTable,
                inputLabel, inputTable);
    }
